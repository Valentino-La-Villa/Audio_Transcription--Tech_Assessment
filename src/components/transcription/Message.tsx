...
export default function Message(props: PropsType) {
    return (
        ...
        ${props.message.role === 'user' 
            ...
            : 'align-self-start justify-content-start'}`} // Aligning user emited messages to the right, and agent emited messages to the left
        >
            ...
            ${(props.message.start < props.currentTimeReference && props.currentTimeReference < props.message.end)
                ...
                : 'opacity-75 border border-black'}
            ${props.message.role === 'user' 
                ? 'bg-primary' 
                : 'bg-danger'}`} // Setting a different color for messages made by the user and the agent
            >
